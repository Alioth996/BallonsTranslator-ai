...
import numpy as np
from qtpy.QtWidgets import QGraphicsOpacityEffect, QLabel, QColorDialog, QMenu, QSizePolicy
...
class ConfigClickableLabel(ClickableLabel):
    def __init__(self, text=None, parent=None, *args, **kwargs):
        ...
        # 设置尺寸策略，允许标签在窗口缩小时自适应
        self.setSizePolicy(QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Preferred)
        # 设置文本对齐方式，使文本居中显示
        self.setAlignment(Qt.AlignmentFlag.AlignCenter)
        # 设置最小宽度，确保文本能够显示
        if text:
            self.setMinimumWidth(len(text) * 6)  # 根据文本长度设置最小宽度
