...
class PageListView(QListWidget):
    ...
    reveal_file = Signal()
    remove_images = Signal(list)  # 新增信号，用于删除选中的图片
    translate_images = Signal(list)  # 新增信号，用于翻译选中的图片
    ...
    def contextMenuEvent(self, e: QContextMenuEvent):
        ...
        if selected_items:
            ...
            translate_act.setEnabled(len(selected_items) > 0)

            menu.addSeparator()
            remove_act = menu.addAction(self.tr('Remove from Project'))
            remove_act.setEnabled(len(selected_items) > 0)
        ...
        if rst == reveal_act:
            ...
            # 收集选中项目的文本用于翻译
            selected_image_names = [item.text() for item in selected_items]
            self.translate_images.emit(selected_image_names)
        elif selected_items and rst == remove_act:
            # 收集选中项目的文本用于删除
            selected_image_names = [item.text() for item in selected_items]
            self.remove_images.emit(selected_image_names)
...
class MainWindow(mainwindow_cls):
    ...
    def setupUi(self):
        ...
        self.canvas.drop_open_files.connect(self.dropOpenFiles)  # 连接拖放图片文件的信号
    ...
    def dropOpenFiles(self, file_paths: list):
        ...
        # 确定目标目录，始终使用第一张图片所在的文件夹
        first_image_path = file_paths[0]
        target_dir = osp.dirname(first_image_path)
        ...
        try:
            ...
            for file_path in file_paths:
                ...
                dest_path = osp.join(target_dir, file_name)

                # 标准化路径，确保使用相同的路径分隔符
                file_path = osp.normpath(file_path)
                dest_path = osp.normpath(dest_path)

                # 检查源文件和目标文件是否相同
                if osp.normcase(file_path) == osp.normcase(dest_path):
                    # 如果是相同文件，直接添加到已复制列表，不进行复制操作
                    copied_files.append(file_name)
                    continue

                try:
                    # 我们不复制文件，而是直接使用原始文件路径
                    # 这样可以避免复制操作可能导致的问题
                    copied_files.append(file_name)
                except Exception as e:
            ...
            for ii, file_name in enumerate(copied_files):
                self.imgtrans_proj.pages[file_name] = []
                self.imgtrans_proj._pagename2idx[file_name] = ii
                self.imgtrans_proj._idx2pagename[ii] = file_name
            ...
            # 更新UI
            self.st_manager.clearSceneTextitems()
            self.titleBar.setTitleContent(osp.basename(target_dir))
            self.updatePageList()

        except Exception as e:
            # 如果出错，不删除用户文件夹，只显示错误信息
            LOGGER.error(f"Error importing images: {str(e)}")
            create_error_dialog(e, self.tr('导入图片失败'))
    ...
    def on_translate_images(self, image_names: list):
        ...
        for img_name in image_names:
            if img_name in self.imgtrans_proj.pages:
                # 切换到要翻译的图片
                self.imgtrans_proj.set_current_img(img_name)
                self.canvas.updateCanvas()
                self.st_manager.updateSceneTextitems()
    ...
    def on_remove_images(self, image_names: list):
        ...
        # 确认删除
        confirm_msg = self.tr('Are you sure you want to remove the selected images from the project?')
