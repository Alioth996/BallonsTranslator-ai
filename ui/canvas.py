...
class Canvas(QGraphicsScene):
    ...
    drop_open_files = Signal(list)  # 新增信号，用于处理拖放的图片文件
    ...
    def dragEnterEvent(self, e: QGraphicsSceneDragDropEvent):
        ...
        if e.mimeData().hasUrls():
            ...
            # 如果有文件夹且没有图片文件，处理文件夹
            elif self.drop_folder is not None:
                e.acceptProposedAction()
    ...
    def updateCanvas(self):
        ...
        # 检查是否有有效图片，如果没有则清空所有图层
        if not self.imgtrans_proj.img_valid:
            # 清空所有图层
            if self.base_pixmap is not None:
                self.base_pixmap = None
            if self.textLayer.pixmap() is not None:
                empty_pixmap = QPixmap(1, 1)
                empty_pixmap.fill(Qt.GlobalColor.transparent)
                self.textLayer.setPixmap(empty_pixmap)
                self.inpaintLayer.setPixmap(empty_pixmap)
                self.drawingLayer.setPixmap(empty_pixmap)
            # 重置场景大小，但保持一个合理的最小尺寸以便拖拽操作
            min_width = 400
            min_height = 300
            self.baseLayer.setRect(QRectF(0, 0, min_width, min_height))
            self.setSceneRect(0, 0, min_width, min_height)
            # 确保拖拽功能仍然可用
            self.gv.setAcceptDrops(True)
            self.inpaintLayer.setAcceptDrops(True)
            self.drawingLayer.setAcceptDrops(True)
