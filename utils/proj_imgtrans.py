    ...
    def set_current_img(self, imgname: str):
        if imgname is not None:
            ...
            if self.img_array is None:
                ...
                # 如果图片读取失败，创建一个小的空白图片
                self.img_array = np.ones((100, 100, 3), dtype=np.uint8) * 255
    ...
    def load_inpainted_by_imgname(self, imgname: str, scale_to_src: bool = True) -> np.ndarray:
        ...
        if mp is not None and osp.exists(mp):
            ...
            if inpainted is None:
                LOGGER.error(f'Failed to read inpainted image: {mp}')
                return None
            ...
            try:
                ...
                if imgname == self.current_img and self.img_array is not None:
                    ...
                    try:
                        ...
                        LOGGER.error(f'Error opening image {imgname}: {str(e)}')
                        return inpainted
                ...
                if ih != h or iw != w:
                    try:
                        ...
                        LOGGER.error(f'Error resizing inpainted image: {str(e)}')
            except Exception as e:
                LOGGER.error(f'Error processing inpainted image: {str(e)}')
